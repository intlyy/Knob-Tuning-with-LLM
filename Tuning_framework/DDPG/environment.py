...
class Database:
    ...
    def fetch_knob(self):
        ...
        try:
            ...
            for i in range(self.knob_num):
                state_list.append(result[0]["@@%s" % self.knob_names[i]])
            cursor.close()
            conn.close()
        except Exception as error:
    ...
    def change_knob_nonrestart(self, actions):
        ...
        #     return 0
        set_knobs_command = '\cp {} {};'.format(self.argus['mysql_cnf_bak'], self.argus['mysql_cnf'])
        for i in range(self.knob_num):
            set_knobs_command += 'echo "{}"={} >> {};'.format(self.knob_names[i], int(actions[i]), self.argus['mysql_cnf'])
        head_command = 'sshpass -p {} ssh {} '.format(self.argus['ssh_pwd'],self.argus['ssh_host'])
...
class Environment(gym.Env):
    ...
    def __init__(self, db, argus):
        ...
        self.state = np.array(db.fetch_internal_metrics())
        ...
        ################################################################################
        state0 = np.array(self.db.fetch_internal_metrics())
        self.preheat()
        state1 = np.array(self.db.fetch_internal_metrics())
        try:
    ...
    def execute_command(self):
        ...
        return run_job(thread_num, self.workload, self.parser.resfile)
    ...
    def step(self, u, isPredicted, iteration, action_tmp=None):
        ...
        if not flag:
            record = {'knobs': u.tolist(), 'qps': -1}
            f = open('result_wab_t120', 'a')
            f.writelines(json.dumps(record) + '\n')
            f.close()
            return self.state, -10e+4, self.score, 1
        throughput, latency = self.execute_command()
        print("iteration: {}\tqps = {}\tlat = {}".format(iteration, throughput, latency))

        record = {'knobs': self.db.fetch_knob(), 'qps': throughput, 'metrics': self.db.fetch_internal_metrics()}
       ...
       '''
        ...
        action = np.array(self.fetch_action())
    ...
    def _get_obs(self):
        self.state = np.array(self.db.fetch_internal_metrics())
        self.state = np.append(self.parser.predict_sql_resource(self.workload), self.state)
        return self.state
