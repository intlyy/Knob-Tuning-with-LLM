...
from configs import get_knob_config
...
class ActorCritic:
    ...
    def _train_critic(self, samples, i):
        for sample in samples:
            ...
            new_state = (new_state - min(new_state[0])) / (max(new_state[0]) - min(new_state[0]))
            ...
            tmp = list()
            for i in target_action[0]:
                tmp.append(float(i))
            print("target action\n{}".format(target_action))
            tmp_action = [tmp]
            tmp_action = np.array(tmp_action)
            tmp_action = (tmp_action - min(tmp_action[0])) / (max(tmp_action[0]) - min(tmp_action[0]))

            print("new state\n{}".format(new_state))
            print("tmp action\n{}".format(tmp_action))

            future_reward = self.target_critic_model.predict([new_state, tmp_action])[0][0]
            print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    ...
    def act(self, cur_state):
        ...
        if np.random.random(1) < self.epsilon or len(self.memory) < self.batch_size:
            if len(self.memory) < self.batch_size:
                print("training data is not enough")
            print("[Random Tuning]")
